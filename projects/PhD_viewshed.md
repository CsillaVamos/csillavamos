...
## Methods
Processing methods included snapping each residential address point to the nearest street (as no data was available on orientation of windows in resiential addresses). Buffers (radii) of 150 meters were then created around each snapped point. A viewshed analysis for each buffer was performed. Average NDVI values using the view-shed as a mask were then calcualted for each buffer. Later analysis compared results to survey answers on local green space perception. 
